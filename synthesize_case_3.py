...
def main():
    ...
    if not os.path.isdir(case_dir):
        os.mkdir(case_dir)
    ligand_diffusivity = np.array([200.0, 200.0, 200.0])
    production_rate_scale = np.array([1.0, 1.0, 1.0])
    receptor_init_scale = np.array([1.0, 1.0, 1.0])
    binding_rate = np.ones([3,3])
    dissociation_rate = np.ones([3,3]) * 0.001
    degradation_rate_ligand = np.array([0.2, 0.2, 0.2])
    xlimit = [-200, 200]
    ylimit = [-200, 200]
    xngrid = 50
    yngrid = 50
