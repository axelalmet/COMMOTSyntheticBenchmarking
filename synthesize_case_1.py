...
def main():
    ...
    receptor_init_scale = np.array([1.0])
    binding_rate = np.array([[1.0],[1.0]])
    dissociation_rate = np.array([[0.001],[0.001]])
    ...
    receptor_nblob = np.array([5], int)
    ligand_blob_bandwidth_bound = np.array([[30,50],[30,50]])
    receptor_blob_bandwidth_bound = np.array([[30,50]])
    np.random.seed(1)
    for i in range(10):
