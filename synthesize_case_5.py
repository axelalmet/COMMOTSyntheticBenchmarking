...
def main():
    ...
    if not os.path.isdir(case_dir):
        os.mkdir(case_dir)
    ligand_diffusivity = 200.0 * np.ones(3)
    production_rate_scale = np.ones(3)
    receptor_init_scale = np.ones(5)
    binding_rate = np.ones([3,5])
    dissociation_rate = np.ones([3,5]) * 0.001
    degradation_rate_ligand = 0.2 * np.ones(3)
    xlimit = [-200, 200]
    ylimit = [-200, 200]
    xngrid = 50
    yngrid = 50
    ligand_nblob = np.array([5,5,5], int)
    receptor_nblob = np.array([5,5,5,5,5], int)
    ligand_blob_bandwidth_bound = np.array([[30,50],[30,50],[30,50]])
    receptor_blob_bandwidth_bound = np.array([[30,50],[30,50],[30,50],[30,50],[30,50]])
    np.random.seed(5)
    for i in range(10):
